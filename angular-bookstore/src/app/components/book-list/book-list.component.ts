...
export class BookListComponent implements OnInit {
  ...
  processResult(){
    return data => {
      this.books = data.content; // Spring Data Page uses 'content' instead of '_embedded.books'
      this.currentPage = data.number + 1; // Page uses 'number' instead of 'page.number'
      this.totalRecords = data.totalElements; // Page uses 'totalElements' instead of 'page.totalElements'
      this.pageSize = data.size; // Page uses 'size' instead of 'page.size'
    }
  }
